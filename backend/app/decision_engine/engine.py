...
class DecisionInput:
    ...
    def dti_ratio(self) -> float:
        ...
        # term_months == 0 means the single-payment-in-30-days option;
        # treat as a 1-month equivalent obligation for affordability purposes.
        effective_months = self.requested_term_months if self.requested_term_months > 0 else 1
        estimated_payment = float(self.requested_amount) / effective_months
...
class RateCalculator:
    """
    Ramus Financial Solutions short-term loan product:
    a single fixed cost-of-credit, not a risk-adjusted annual rate.
    Loan amount is capped at LOAN_MAX_AMOUNT (see app.core.config).
    """

    FIXED_COST_OF_CREDIT_PERCENT = Decimal("30.00")

    def calculate(self, risk_category: str, requested_amount: Decimal) -> Decimal:
        # Fixed for all risk categories — risk is handled via the approve/decline/
        # manual_review rules above, not via a variable rate.
        return self.FIXED_COST_OF_CREDIT_PERCENT
...
class RepaymentCalculator:
    """
    Computes repayment figures for the Ramus Financial Solutions loan product.

    Two repayment structures:
      1. Single payment — full amount + 30% cost of credit, due in 30 days.
      2. Instalments — 1 to 6 months, principal + 30% cost of credit divided
         evenly across the chosen number of months.
    """

    COST_OF_CREDIT_PERCENT = Decimal("30.00")

    def total_repayable(self, principal: Decimal) -> Decimal:
        return (principal * (Decimal("1") + self.COST_OF_CREDIT_PERCENT / Decimal("100"))).quantize(Decimal("0.01"))

    def single_payment_plan(self, principal: Decimal) -> dict:
        total = self.total_repayable(principal)
        return {
            "plan": "single_payment",
            "principal": principal,
            "cost_of_credit_percent": self.COST_OF_CREDIT_PERCENT,
            "total_repayable": total,
            "due_in_days": 30,
            "instalments": 1,
            "instalment_amount": total,
        }

    def instalment_plan(self, principal: Decimal, months: int) -> dict:
        if months < 1 or months > 6:
            raise ValueError("Instalment term must be between 1 and 6 months")
        total = self.total_repayable(principal)
        instalment = (total / Decimal(months)).quantize(Decimal("0.01"))
        return {
            "plan": "instalments",
            "principal": principal,
            "cost_of_credit_percent": self.COST_OF_CREDIT_PERCENT,
