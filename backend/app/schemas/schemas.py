...
class ApplicationCreate(BaseModel):
    # Ramus Financial Solutions product: max loan R5,000, 30% fixed cost of credit.
    # requested_term_months: 0 = single payment in full after 30 days, 1-6 = instalment plan.
    requested_amount: Decimal = Field(..., gt=0, le=5000, description="Maximum loan amount is R5,000")
    requested_term_months: int = Field(..., ge=0, le=6, description="0 = pay in full after 30 days, 1-6 = monthly instalments")
...
class DecisionOverrideRequest(BaseModel):
    ...
    approved_amount: Optional[Decimal] = Field(None, gt=0, le=5000)
    approved_term_months: Optional[int] = Field(None, ge=0, le=6)
