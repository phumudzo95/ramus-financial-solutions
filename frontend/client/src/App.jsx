...
const MOCK_APPS = [
  { ref: "RCL-X7K29MQA", amount: 15000, status: "manual_review", status_label: "Under Review", submitted: "10 Jun 2026", updated: "11 Jun 2026" },
];
...
export default function ClientPortal() {
  ...
  const [form, setForm] = useState({ amount: "", term: "12", purpose: "", income: "", expenses: "", employer: "", employment_type: "employed" });
  const [submitted, setSubmitted] = useState(false);

  const update = (k, v) => setForm(f => ({ ...f, [k]: v }));
  const monthlyPayment = form.amount && form.term ? Math.round((parseFloat(form.amount) * 1.18) / parseInt(form.term)) : null;
  ...
  return (
    <div style={{ fontFamily: "system-ui, sans-serif", minHeight: "100vh", background: "#F9FAFB" }}>
      {/* Header */}
      <div style={{ background: NAVY, display: "flex", alignItems: "center", gap: 14, padding: "14px 24px", borderBottom: `3px solid ${GOLD}` }}>
        ...
        <div>
          <div style={{ fontWeight: 800, fontSize: 17, color: "#fff", letterSpacing: 0.5 }}>RAMUS CASH LOANS</div>
        ...
        <div style={{ marginLeft: "auto", display: "flex", gap: 8 }}>
          {["home", "apply", "status"].map(v => (
            <button key={v} onClick={() => setView(v)} style={{ padding: "8px 18px", background: view === v ? GOLD : "transparent", color: view === v ? NAVY : "#fff", border: `1px solid ${view === v ? GOLD : "rgba(255,255,255,0.3)"}`, borderRadius: 8, cursor: "pointer", fontWeight: view === v ? 700 : 400, fontSize: 13 }}>
        ...
        <div>
          ...
          <div style={{ maxWidth: 780, margin: "48px auto", padding: "0 24px" }}>
            ...
            <div style={{ background: "#fff", borderRadius: 14, border: "1px solid #E5E7EB", padding: 28, marginTop: 28 }}>
              ...
              <p style={{ margin: "0 0 20px", color: "#6B7280", fontSize: 13 }}>Estimate your monthly repayment</p>
              <div style={{ display: "flex", gap: 20, alignItems: "flex-end" }}>
                <div style={{ flex: 1 }}>
                  <label style={labelStyle}>Loan Amount (ZAR)</label>
                  <input type="number" value={form.amount} onChange={e => update("amount", e.target.value)} placeholder="e.g. 10000" style={inputStyle} />
                </div>
                <div style={{ flex: 1 }}>
                  <label style={labelStyle}>Term (months)</label>
                  <select value={form.term} onChange={e => update("term", e.target.value)} style={inputStyle}>
                    {[6,12,18,24,36,48,60].map(m => <option key={m}>{m}</option>)}
                  </select>
                </div>
                {monthlyPayment && (
                  <div style={{ textAlign: "center", padding: "10px 24px", background: "#F0FDF4", border: "2px solid #BBF7D0", borderRadius: 10 }}>
                    <div style={{ fontSize: 22, fontWeight: 700, color: "#15803D" }}>R {monthlyPayment.toLocaleString()}</div>
                    <div style={{ fontSize: 12, color: "#6B7280" }}>per month (est.)</div>
                  </div>
                )}
              </div>
              <div style={{ fontSize: 11, color: "#9CA3AF", marginTop: 12 }}>Estimate based on 18% p.a. Final rate depends on credit profile. Subject to approval.</div>
            </div>
          </div>
        </div>
      )}
            ...
            {STEPS.map((s, i) => (
              <div key={s} style={{ display: "flex", alignItems: "center", flex: i < STEPS.length - 1 ? 1 : 0 }}>
                <div style={{ display: "flex", flexDirection: "column", alignItems: "center" }}>
                  ...
                  <div style={{ fontSize: 11, color: i === step ? NAVY : "#9CA3AF", marginTop: 4, whiteSpace: "nowrap" }}>{s}</div>
                </div>
          ...
          <div style={{ background: "#fff", borderRadius: 14, border: "1px solid #E5E7EB", padding: 30 }}>
            ...
            {step === 1 && (
              <div>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16, marginBottom: 18 }}>
                  <div>
                    <label style={labelStyle}>Amount Requested (R)</label>
                    <input type="number" value={form.amount} onChange={e => update("amount", e.target.value)} placeholder="e.g. 10000" style={inputStyle} />
                  </div>
                  <div>
                    <label style={labelStyle}>Term (months)</label>
              ...
              <div>
                ...
                <label style={{ display: "flex", gap: 14, cursor: "pointer", alignItems: "flex-start" }}>
                  ...
                  <span style={{ fontSize: 14, color: "#111827", lineHeight: 1.5 }}>
                    I have read and understand the above consent declaration and <strong>authorise Ramus Cash Loans to access my credit profile</strong>.
                  </span>
                </label>
            ...
            {step === 3 && (
              <div>
                ...
                {[["Requested Amount", `R ${parseFloat(form.amount || 0).toLocaleString()}`], ["Loan Term", `${form.term} months`], ["Employment Type", form.employment_type], ["Consent Granted", consentChecked ? "Yes ✓" : "Not yet"]].map(([k, v]) => (
      ...
      <div style={{ background: NAVY, color: "rgba(255,255,255,0.6)", padding: "20px 24px", textAlign: "center", fontSize: 12, marginTop: 40 }}>
        Ramus Cash Loans (Pty) Ltd — Registered Credit Provider | NCR Registration: NCRCP XXXX | Subject to the National Credit Act 34 of 2005
      </div>
    </div>
  );
}
